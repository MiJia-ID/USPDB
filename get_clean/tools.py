...
def _unit_edges_from_block_edges(atom_positions, src_dst, device,k=None):
    """
    直接处理原子级别的边，模拟块转换为原子的逻辑。

    :param atom_positions: [N, 3] 原子的位置坐标
    :param src_dst: [E, 2] 表示原子级别的边（每行是一个 (src, dst)）
    :param k: 如果提供，则每个原子仅连接到最近的 k 个原子
    :return: (unit_src, unit_dst), (edge_id, unit_edge_src_start, unit_edge_src_id)
    """
    # num_atoms = atom_positions.shape[0]
    #
    # # 模拟 `block_n_units`，这里每个原子就是一个单元
    # atom_n_units = torch.ones(num_atoms, dtype=torch.long)  # 每个原子都是独立的单元
    # atom_offsets = torch.arange(num_atoms)  # 每个原子都是单独的块，偏移量是0到N-1

    # 获取边的端点数目，直接基于原子级别的连接
    edge_n_units = torch.ones(src_dst.shape[0], 2, dtype=torch.long)  # 每条边连接的就是 (1, 1) 单元对
    edge_n_pairs = edge_n_units[:, 0] * edge_n_units[:, 1]  # 每条边只有 1 个原子对

    # 模拟 `edge_id`，每个边对应该边的索引
    edge_id = torch.arange(edge_n_pairs.sum(), dtype=torch.long)  # 每对单元对的边的唯一ID

    # 计算单元对的索引
    unit_src_dst = torch.ones_like(edge_id)
    if len(unit_src_dst) > 0:
        unit_src_dst[0] = 0  # 设置初始位置
    unit_src_dst = torch.cumsum(unit_src_dst, dim=0)  # 单元对的累积索引

    # 直接利用偏移量（这里偏移量是每个原子的索引，因为没有块的概念）
    unit_src = src_dst[:, 0].to(device)
    unit_dst = src_dst[:, 1].to(device)

    unit_edge_src_start = (unit_dst == 0)  # 标记边的开始位置
    unit_edge_src_id = unit_edge_src_start.long()
    if len(unit_edge_src_id) > 1:
        unit_edge_src_id[0] = 0
    unit_edge_src_id = torch.cumsum(unit_edge_src_id, dim=0)  # 累加边的起始ID

    if k is None:
        return (unit_src, unit_dst), (edge_id, unit_edge_src_start, unit_edge_src_id)

    # 稀疏化：如果提供了 k，保留距离最近的 k 个原子
    D = torch.norm(atom_positions[unit_src] - atom_positions[unit_dst], dim=-1)
    max_n = torch.max(scatter_sum(torch.ones_like(unit_edge_src_id), unit_edge_src_id))
    k = min(k, max_n)

    BIGINT = 1e10  # 赋予无效边一个大的距离值
    N = unit_edge_src_id.max() + 1

    dist_mat = torch.ones(N, max_n, device=D.device, dtype=D.dtype) * BIGINT  # 初始化为大值
    dist_mat[(unit_edge_src_id, unit_dst)] = D  # 填充真实距离
    dist_neighbors, dst = torch.topk(dist_mat, k, dim=-1, largest=False)  # 获取最近的 k 个邻居

    src = torch.arange(0, N, device=dst.device).unsqueeze(-1).repeat(1, k)
    unit_edge_src_start = torch.zeros_like(src).bool()
    ...
    mat[(unit_edge_src_id, unit_dst)] = unit_src
    unit_src = mat[(src, dst)]
    mat[(unit_edge_src_id, unit_dst)] = unit_dst
    unit_dst = mat[(src, dst)]
    mat[(unit_edge_src_id, unit_dst)] = edge_id
    edge_id = mat[(src, dst)]
