...
class GET(nn.Module):
    ...
    def __init__(self, d_hidden, d_radial, n_channel, n_rbf, cutoff=7.0, d_edge=0, n_layers=4, n_head=4,
        ...
        self.recover_layer = nn.Linear(128, 6205)
    ...
    def recover_scale(self, Z, block_id, batch_id, record_scale):
        ...
        record_scale_expanded = record_scale.squeeze(0)  # shape: [3, 1]
        record_scale_expanded = record_scale_expanded[unit_batch_id]  # shape: [297, 1]
        record_scale_final = record_scale_expanded.expand(-1, 3)  # shape: [297, 3]
        Z = Z_c + Z_centered /  record_scale_final

        return Z

    def forward(self, H, Z, block_id, batch_id, edges,device, edge_attr=None,cached_unit_edge_info=None):
        if cached_unit_edge_info is None:
            with torch.no_grad():
                #cached_unit_edge_info = _unit_edges_from_block_edges(Z,edges,device, k=self.sparse_k) # [Eu], Eu = \sum_{i, j \in E} n_i * n_j
                cached_unit_edge_info = _unit_edges_from_block_edges(Z, edges, device)
        ...
        for i in range(self.n_layers):
            H, Z = self._modules[f'layer_{i}'](
                H, Z, block_id, edges, device, edge_attr, cached_unit_edge_info
            )
            H, Z, rescale = self._modules[f'layernorm0_{i}'](H, Z, block_id, batch_id, device)
            record_scale *= rescale
            H, Z = self._modules[f'ffn_{i}'](H, Z, block_id, device)
            H, Z, rescale = self._modules[f'layernorm1_{i}'](H, Z, block_id, batch_id, device)
            record_scale *= rescale
            H = self.recover_layer(H)
...
class GETLayer(nn.Module):
    ...
    def __init__(self, d_hidden, d_radial, n_channel, n_rbf, cutoff=7.0,
        ...
        d_hidden_head, d_radial_head = d_hidden , d_radial // self.n_head
        n_rbf_head = n_rbf // self.n_head
        self.linear_h = nn.Linear(6205, d_hidden_head)
        self.linear_qk = nn.Linear(d_hidden_head // self.n_head, d_radial_head * 2, bias=False)
        self.linear_v = nn.Linear(d_hidden_head // self.n_head, d_radial_head)
    ...
    def attention(self, H, Z, edges, edge_attr, device,cached_unit_edge_info):
        # row, col = edges
        row = edges[:, 0].unsqueeze(dim=-1)  # [N, 1]
        row = row.to(device)
        col = edges[:, 1].unsqueeze(dim=-1)  # [N, 1]
        (unit_row, unit_col), (block_edge_id, unit_edge_src_start, unit_edge_src_id) = cached_unit_edge_info
        (block_edge_id, unit_edge_src_start, unit_edge_src_id) = (
            block_edge_id.to(device),
            unit_edge_src_start.to(device),
            unit_edge_src_id.to(device)
        )
        # multi-head
        if H.shape[1] == 6205:
            H = self.linear_h(H)
        H = H.view(H.shape[0], self.n_head, int(H.shape[1] / self.n_head)) # [N, n_head, hidden_size / n_head]
        # calculate attention
        ...
        if self.n_rbf > 1:
            n_channel = D.shape[-1]
            D = self.rbf(D.view(-1)).view(D.shape[0], self.n_channel, self.n_head, int(self.n_rbf / self.n_head))  # [Eu, n_channel, n_head, n_rbf / n_head]
        ...
        R_repr = R_repr.to(torch.float32)
    ...
    def equivariant_update(self, H_v, Z, alpha, beta, D, dZ, cached_unit_edge_info):
        ...
        Z_agg = scatter_sum(
            (alpha * self.unit_msg_coord_linear(H_v)) * dZ.unsqueeze(1),
        ...
        Z_agg = scatter_sum(
            (beta * Z_H_agg)* Z_agg, unit_agg_row,
    ...
    def forward(self, H, Z, block_id, edges, device, edge_attr=None, cached_unit_edge_info=None):
        '''
        H: [N, hidden_size],
        Z: [N, n_channel, 3],
        block_id: [N],
        ...
        alpha, beta, (D, R, dZ) = self.attention(H, Z, edges, edge_attr,device, cached_unit_edge_info)
...
class EquivariantFFN(nn.Module):
    ...
    def forward(self, H, Z, block_id,device):
        '''
        :param H: [N, d_in]
        ...
        AdaptiveLinearLayer = nn.Linear(input_features, 272).to(device)
        inputs = AdaptiveLinearLayer(inputs)
...
class EquivariantLayerNorm(nn.Module):
    ...
    def __init__(self, d_hidden, n_channel, n_rbf=16, cutoff=7.0, act_fn=nn.SiLU()) -> None:
        ...
        self.fuse_scale_ffn = nn.Sequential(
            nn.Linear(n_channel * n_rbf, d_hidden).double(),
            act_fn,
            nn.Linear(d_hidden, d_hidden).double(),
            act_fn
        )
        self.layernorm = nn.LayerNorm(d_hidden).double()
    ...
    def forward(self, H, Z, block_id, batch_id, device):
        with torch.no_grad():
            dims = Z.dim()
            # if dims == 2:
            #     Z = Z.unsqueeze(0)
        ...
        # elif dims == 2:
        var = scatter_std(
            #Z_centered.transpose(0, 1).reshape(-1, n_channel).contiguous(),
            Z_centered.transpose(0, 1).reshape(-1),
            unit_axis_batch_id, dim=0)  # [bs, n_channel]
        ...
        # if dims == 3 :
        #     input_features = rescale_rbf.size(1)
        #     if num == 1:
        #         AdaptiveLinearLayer = nn.Linear(input_features, 16).to(torch.float64).to(device)
        #         rescale_rbf = AdaptiveLinearLayer(rescale_rbf)
        #         num = num + 1
