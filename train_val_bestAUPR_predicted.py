...
########################修改模型保存路径################################
# predicted structure
Model_Path = '/home/mijia/EGPDI/Model/multi-head/thirty_two'
...
save_edgefeats_path = root_dir + '/Train_Test129/edge_features/EdgeFeats_predicted_SC_17.pkl'
...
def train_one_epoch(model,data_loader):
    ...
    for label_batch, node_features_batch, graph_batch,efeat_batch,adj_batch,coors_batch in data_loader:
        ...
        y_pred = model(graph_batch, node_features_batch,coors_batch,adj_batch,efeat_batch,device)
...
def cross_validation(all_dataframe,fold_number = 5):
    ...
    for train_index,valid_index in kfold.split(sequence_names,sequence_labels):
        ...
        # model = U_MainModel(dr,lr,nlayers,lamda,alpha,atten_time,nfeats=41+20+1024+5120)
        model = U_MainModel(dr, lr, nlayers, lamda, alpha, atten_time, nfeats=41 + 20 + 1024 + 5120)
        model.to(device)
        # if torch.cuda.is_available():
        #     model.cuda()
