...
from model_with_edge_features import U_MainModel
...
parser.add_argument("--edgefeats_path", type=str, default='/home/mijia/EGPDI/data/dataset_dir_181/Test_181/edge_features/EdgeFeats_predicted_SC_17_181.pkl')
parser.add_argument("--model_path", type=str, default='/home/mijia/EGPDI/Model/U-EGNNet/')
...
pkl_path = root_dir + 'dataset_dir_181/PDNA_residue_feas_PSA.pkl'
...
X, y = create_dataset(query_ids, test_path, pkl_path, esm2_5120_path, ProtTrans_path,
...
test_dic = {"ID": IDs, "sequence": sequences, "label": labels, 'features': features, 'coors': coors, 'adj': adjs,
            'graph': graphs, 'efeats': efeats}
...
def evaluate(model, data_loader):
    ...
    for label_batch, node_features_batch, graph_batch, efeat_batch, adj_batch, coors_batch in data_loader:
        with torch.no_grad():
            ...
            y_pred = model(graph_batch, node_features_batch, coors_batch, adj_batch, efeat_batch, device).to(device)

            softmax = torch.nn.Softmax(dim=1)
            y_pred = softmax(y_pred)
            y_pred = torch.squeeze(y_pred)
            y_pred_181.append(y_pred)
    ...
    # with open('y_pred_181_for_pymol.txt', 'w') as f:
    #     for index, value in enumerate(y_pred_181):
    #         f.write(f"索引: {index}, 元素: {value}")
    #     print('已写入')
